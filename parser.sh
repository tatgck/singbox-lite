...
_parse_ss() {
    ...
    # 检查是否为 SIP002 格式: ss://base64(method:password)@server:port
    if [[ "$body" == *"@"* ]]; then
        local userinfo="${body%%@*}"
        server_port="${body#*@}"
        ...
        if [[ "$decoded_userinfo" == *":"* ]]; then
            # 已经是明文 method:password 格式
            method_pass="$decoded_userinfo"
        else
            # 尝试 base64 解码
            method_pass=$(_decode_base64_urlsafe "$userinfo")
            if [ -z "$method_pass" ]; then
                echo '{"error": "Shadowsocks userinfo Base64 解码失败"}'
                return 1
            fi
        fi
    else
        ...
        if [ -z "$decoded" ]; then
            echo '{"error": "Shadowsocks Base64 解码失败"}'
            return 1
        fi

        # 检查是否包含 @
        if [[ "$decoded" != *"@"* ]]; then
            echo '{"error": "Shadowsocks 链接格式错误：缺少 @ 分隔符"}'
            return 1
        fi
    ...
    # 验证 method_pass 格式
    if [[ "$method_pass" != *":"* ]]; then
        echo '{"error": "Shadowsocks 认证信息格式错误：缺少 method:password"}'
        return 1
    fi

    # 分割主机和端口
    local split_result
    split_result=$(_split_host_port "$server_port")
    if [ $? -ne 0 ] || [ -z "$split_result" ]; then
        echo '{"error": "Shadowsocks 服务器地址或端口格式错误"}'
        return 1
    fi
    ...
    # 提取 method 和 password
    local method="${method_pass%%:*}"
    local password="${method_pass#*:}"

    # 验证必要字段
    if [ -z "$server" ] || [ -z "$port" ] || [ -z "$method" ] || [ -z "$password" ]; then
        echo '{"error": "Shadowsocks 链接信息不完整"}'
        return 1
    fi

    # 生成 JSON 输出
    jq -n --arg s "$server" --argjson p "$port" --arg m "$method" --arg pw "$password" \
