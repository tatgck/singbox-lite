...
`[5] 修改节点` 提供端口与 SNI 两项修改。修改 SNI 支持 Reality 与 TLS 类节点：Reality 节点同步更新握手伪装域名，自签证书节点可联动按新 SNI 重新生成证书并更新链接中的证书指纹；保存时统一校验 sing-box 配置并同步元数据、分享链接与 Clash/Mihomo 配置，失败自动回滚。

`[20] SNI 优选` 按目标区域（美国/日本/新加坡，或自动检测本机所在地区）对内置候选域名池做 3 轮 TLS 握手延迟测试（优选标准：TLS 1.3 + HTTP/2、直连可通、延迟低且稳定，思路参考 XTLS/RealiTLScanner），按延迟与稳定性综合评分输出前 5 名（倒序展示，最佳在最后），可直接用于节点或中转入口的 SNI 修改。
...
进阶转发菜单 `[7] 修改中转入口 SNI` 可按序号选择已有中转，展示入口协议、端口、当前 SNI 与落地信息后修改 SNI：Reality 入口同步更新握手伪装域名，Hysteria2/TUIC/AnyTLS 入口按新 SNI 重新生成自签证书并更新链接指纹；合并配置校验失败自动回滚，成功后重启服务并输出更新后的分享链接。
...
- 适配 sing-box 1.14：DNS 配置迁移为类型化服务器格式并写入 `route.default_domain_resolver`，存量旧格式配置在脚本启动时自动迁移，修复 1.14 核心启动 FATAL。
- 主菜单 `[5]` 扩展为「修改节点端口/SNI」：新增按编号修改节点 SNI，Reality 节点同步更新握手伪装域名，自签证书节点可联动重新生成证书并更新分享链接与 Clash/Mihomo 配置，失败自动回滚。
- 进阶转发菜单新增 `[7] 修改中转入口 SNI`：支持四种中转入口协议，展示入口/端口/落地信息后按序号修改，证书入口自动按新 SNI 重签并更新链接指纹（原「清空所有中转配置」调整为 `[8]`，「端口转发管理」调整为 `[9]`）。
- 主菜单新增 `[20] SNI 优选`：按目标区域（美国/日本/新加坡或自动检测本机地区）对候选域名池做 3 轮 TLS 握手延迟测试，综合延迟与稳定性输出前 5 名（倒序展示，最佳在最后）。